...
import TopicsGroup from "@/components/Focus/Topic/TopicsGroup";
...
export default function Focus({ language }: Props) {
    ...
    const focusPauses = focusMinutes
        ? Math.max(1, Math.floor(focusMinutes / 25))
        : 1;
    ...
    const TITLE_MISSING = TranslateText(
        "You forgot to give the focus a title!"
    );
    const CATEGORY_MISSING = TranslateText(
        "You forgot to choose a focus category!"
    );
    ...
    function StartFocus(event: React.FormEvent<HTMLFormElement>) {
        ...
        if (data.focusName && topicIdRef.current !== -1) {
            ...
            startNewFocus(
                data.focusName,
                topicIdRef.current,
                focusMinutes ? focusMinutes * 60 : 60 * 60
            );
        }
    }
    ...
    function createNotification(title: string, message: string) {
        if (hasNotificationPermission && screen.width > 500) {
            ...
            notification = new Notification(title, {
                body: message,
                icon: "/images/notification_image.png",
            });
        }
    }
    ...
    const Focus1 = (
        <div
            ...
            style={{
                gap: "1rem" /* justifyContent: "space-between", height: focusFrameSize */,
            }}
        >
            {errorMessage !== "" ? (
                <motion.div
                ...
                >
                    <header>
                        <div className={styles.column}>
                            <h5>
                                <Translate>Wait a little bit!</Translate>
                            </h5>
                ...
                >
                    <header>
                        <div className={styles.column}>
                            <h5>
                                <Translate>Something's missing...</Translate>
                            </h5>
                            <div className={styles.iconHolder}>
                                <p>
                                    <Translate>
                                        To get the most out of Focus we need you
                                        to allow us to send you notifications!
                                    </Translate>
                                </p>
                            </div>
                        </div>
                ...
                <>
                    <Input
                        label={TranslateText("Task name")}
                        name="focusName"
                        placeholder={TranslateText("Insert the task name here")}
                        height={"3.85rem"}
                        maxLength={25}
                    />
                    ...
                    <div /* style={{ gap: "2.5rem" }} */ className="row">
                        <Button
                            icon={"av_timer"}
                            title={TranslateText("Begin Focus")}
                            preset="sendForm"
                        />
                        <Separator decorative orientation="vertical" />
                        <p className={styles.intervalCount}>
                            <Translate>You will have</Translate> <br />
                            <span>
                                {focusPauses} <Translate>break</Translate>
                                {focusPauses !== 1 && "s"}
                            </span>
                        </p>
                    </div>
    ...
    const Focus2 = ({ focus }: { focus: Focus }) => {
        ...
        const timePerSection =
            focus.totalTime /
            60 /
            (focusPauses === 1 ? focusPauses + 1 : focusPauses);

        const isVisiblyPaused = isPaused && status !== "interval";
        const actualSection =
            Math.floor((timePerSection * 60) / actualCounter) + 1;
                ...
                <header>
                    {status === "interval" ? (
                        <>
                            <div className={styles.column}>
                                <h5>
                                    <Translate>Break time!</Translate>
                                </h5>
                                <div className={styles.iconHolder}>
                                    <p>
                                        <Translate>
                                            You're doing well! But you're still
                                            not made of steel, so get off the
                                            chair and go drink some water :)
                                        </Translate>
                                    </p>
                                </div>
                            </div>
                        ...
                        <>
                            <div className={styles.column}>
                                ...
                                <div className={styles.iconHolder}>
                                    <p>
                                        <Translate>
                                            Congrats on staying focused this
                                            far.
                                        </Translate>
                                    </p>
                                </div>
                        ...
                        <>
                            ...
                            >
                                ...
                                <div className={styles.iconHolder}>
                                    ...
                                    <p>{`${focus.totalTime / 60} minuto${
                                        focus.totalTime / 60 !== 1 ? "s" : ""
                                    }`}</p>
                                </div>
                            </div>
                            <div className={styles.info}>
                                ...
                                >
                                    <h6>
                                        {actualSection}/{focusPauses + 1}
                                    </h6>
                                    {focusPauses + 1 > 1 && (
                                        <p>
                                            {Math.floor(timePerSection)}{" "}
                                            <Translate>minute</Translate>
                                            {Math.floor(timePerSection) !== 1
                                                ? "s"
                                                : ""}
                                        </p>
                                    )}
                    ...
                    <div className={styles.clockHolder}>
                        <div className={styles.clock}>
                            <div
                                ...
                                style={{
                                    color: "red",
                                    background: `conic-gradient(var(--primary-01) ${
                                        (actualCounter * 360) /
                                        (status === "interval"
                                            ? intervalTime
                                            : focus.totalTime)
                                    }deg, var(--primary-03) 90deg, var(--primary-02) 180deg, var(--primary-02) 270deg)`,
                                }}
                            />
                            <h3
                                style={{
                                    fontSize:
                                        timeString.length > 5
                                            ? "2rem"
                                            : "2.4rem",
                                    opacity: isVisiblyPaused ? 0.25 : 1,
                                }}
                            >
                                {timeString}
                            </h3>
                            {isVisiblyPaused && (
                                <span
                                    onClick={() => stopCounter()}
                                    className={`${styles.pauseIcon} material-symbols-rounded static click instantFilled`}
                                >
                                    stop
                                </span>
                            )}
    ...
    return (
        <AnimatePresence mode="sync" initial={false}>
            ...
            >
                ...
                >
                    <h3>
                        <Translate>Focus</Translate>
                    </h3>
                    {currentFocus && (
                        <div className={styles.focusInfo}>
                            <p className={styles.emoji}>
                                {focusData[currentFocus.topicId].icon}
                            </p>
                ...
                </motion.div>
                {
                    currentFocus !== null ? (
                        ...
                        Focus1
                    ) // não pode ser um componente <Focus /> por conta da atualização do state que tira o foco do input
                }
            </form>
        </AnimatePresence>
    );
