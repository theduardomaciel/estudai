...
const filteredIcons = iconsData.filter(
	(icon, index) =>
		bannedIcons.includes(icon.name)
			? false
			: true &&
			  !icon.name.includes("0") &&
			  !icon.name.includes("1") &&
			  !icon.name.includes("2") &&
			  !icon.name.includes("3") &&
			  !icon.name.includes("4") &&
			  !icon.name.includes("5") &&
			  !icon.name.includes("6") &&
			  !icon.name.includes("7") &&
			  !icon.name.includes("8") &&
			  !icon.name.includes("9") /* !icon.name.match(/^\d/) */
);
// removemos os ícones banidos e os ícones que começam com número

export default function IconPickerModalPreset(
	...
	const icons = filteredIcons.filter((icon, i) =>
		search.length > 0
			? icon.name.includes(search) || icon.tags.includes(search)
			: true
	);
					...
					{icons.length > 0 ? (
						...
						<EmptyTasksMessage
							...
							description={TranslateText(
								"We couldn't find icons that match your search."
							)}
						/>
					)}
				</ul>
