...
export default function NewSubjectModalPreset(
	previousSubjectsData?: SubjectsData,
	...
	const { IconPickerModal, setIconPickerModalVisible } =
		IconPickerModalPreset(setNewSubjectIcon);
		...
		try {
			...
			if (response.status === 200) {
				...
				if (previousSubjectsData && setSubjectsData) {
					let subjectsDataCopy = Object.assign(
						previousSubjectsData
					) as SubjectsData;
		...
		NewSubjectModal: (
			<Modal
				...
				actionProps={{
					...
					buttonIcon: "add",
					disabled:
						newSubjectName.length < 4 ||
						newSubjectIcon === defaultIcon,
					function:
						modalStatus === true ? CreateNewSubject : undefined,
				}}
				suppressReturnButton={modalStatus === true}
				title={
					modalStatus === "success"
						? TranslateText("Subject created with success")
						: modalStatus === "error"
						? TranslateText("We were unable to create your subject")
						: TranslateText("Create new subject")
				}
			>
				{modalStatus === true ? (
					<>
						<Input
							...
							type="text"
							onChange={(event) =>
								setNewSubjectName(event.currentTarget.value)
							}
						/>
						<Button
							title={
								newSubjectIcon === defaultIcon
									? TranslateText("Select icon")
									: TranslateText("Change icon")
							}
							icon={newSubjectIcon}
							isLoading={isLoading}
							style={{
					...
					<p style={{ color: "var(--primary-02)" }}>
						{modalStatus === "success"
							? TranslateText(
									"Now you can select it in new tasks."
							  )
							: TranslateText(
									"Unfortunately we were unable to create your new subject."
							  )}
					</p>
				)}
			</Modal>
