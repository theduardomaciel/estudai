...
export const Tag = React.forwardRef(
	({ index, tagId, tagType, ...rest }: Props, ref) => {
		const typedRef = ref as RefObject<HTMLLIElement>;
		const [name, icon] = getTagInfo(tagId);

		return (
			<li key={index} className={styles.tag} ref={typedRef} {...rest}>
...
export default function AttachmentTag({ tagId, index, tagType }: Props) {
	...
	return (
		<Tag
			...
			style={
				html5Props.isDragging || touchProps.isDragging
					? { border: `0.5px dashed var(--neutral)`, opacity: 0.5 }
					: { border: `1px solid var(--primary-02)`, opacity: 1 }
			}
		/>
	);
}
