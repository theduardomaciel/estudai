    ...
    return (
        <li key={index} className={fileStyles.attachment}>
            <div className={fileStyles.header}>
                <span style={{ color: "var(--primary-02)" }} className="material-icons-rounded">link</span>
                {
                    links && setLinks &&
                    <span className={`material-icons-rounded ${fileStyles.close}`} onClick={removeLink}> close  </span>
                }
            </div>
            <a
                href={link}
