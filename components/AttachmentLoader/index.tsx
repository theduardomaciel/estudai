...
// Icons
import UploadFileIcon from "@material-symbols/svg-600/rounded/upload-file.svg";
...
export default function AttachmentsLoader({
	...
	const { hasGoogleAuthentication } = { hasGoogleAuthentication: true };
	...
	const TagsPanel = () => {
		...
		return (
			<div id="tagsScroll" className={styles.tagsHolder}>
				<div>
	...
	return (
		<div className={createTaskStyles.column} {...rest}>
			...
			{hasGoogleAuthentication ? (
				...
				>
					{!hasFiles && !hasLinks && (
						<div
							...
							style={{ width: hasFiles ? "100%" : "fit-content" }}
						>
				...
				<div className={`${styles.attachmentHolder} ${styles.expired}`}>
					<span
						className={`${styles.icon} static material-icons-rounded`}
					>
					...
					<div className="">
						{!hasGoogleAuthentication ? (
							<>
								<p>
									Jeez! Looks like our access to your Google
									account has expired :(
								</p>
								<p>
									To attach files again, enter the platform
									again to revalidate your data.
								</p>
							</>
						) : (
							<>
								<p>
									It looks like we had an internal server
									problem.
								</p>
								<p>
									Please try restarting the page to resolve
									the error.
								</p>
							</>
						)}
			...
			<Modal
				...
				icon={LinkIcon}
				toggleVisibility={() =>
					setLinksModalVisible(!isLinksModalVisible)
				}
				dict={{ cancel: "Cancel", return: "Return" }}
				title={`Insert the link below:`}
				actionProps={{
					buttonText: "Add",
					function: () => {
						if (
							linkInputValue.current.length > 10 &&
							isValidUrl(linkInputValue.current)
						) {
			...
			>
				<Input
					onChange={(event) =>
						(linkInputValue.current = event.currentTarget.value)
					}
					type={"url"}
					width={"100%"}
					height={"5rem"}
